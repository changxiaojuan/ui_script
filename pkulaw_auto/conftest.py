...
@pytest.fixture(scope='function')
def sendmail_address():
    return RunConfig.sendmail_address

@pytest.fixture(scope='function')
def sendmail_password():
    return RunConfig.sendmail_password

@pytest.fixture(scope='function')
def receivemail_address():
    return RunConfig.receivemail_address
...
def pytest_runtest_makereport(item):
    ...
    if report.when == 'call' or report.when == "setup":
        ...
        if (report.skipped and xfail) or (report.failed and not xfail):
            ...
            # 原代码对路径包含]进行了判断取名，但实际上没必要，我给注释了
            '''
...
def capture_screenshots(case_name):
    ...
    print("new_report_dir:",new_report_dir)
